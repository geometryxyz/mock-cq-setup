...
use mock_cq_setup::{
    compute_g_powers, compute_tau_powers, lagrange_commitments,
    lagrange_openings_commitments_at_zero, deserialize_vec, compute_qs
};
use std::env;
...
fn run<E: Pairing>(n: usize, tau: E::ScalarField, table_path: &str) {
    ...
    println!("Running compute_tau_powers() took {} seconds.", elapsed_time.as_secs());

    let now = Instant::now();
    let _ = lagrange_commitments::<E::G1>(tau, n as u64);
    let elapsed_time = now.elapsed();
    println!("Running lagrange_commitments() took {} seconds.", elapsed_time.as_secs());

    let now = Instant::now();
    let _ = lagrange_openings_commitments_at_zero::<E::G1>(tau, n);
    let elapsed_time = now.elapsed();
    println!("Running lagrange_openings_commitments_at_zero() took {} seconds.", elapsed_time.as_secs());

    let now = Instant::now();
    let _ = compute_g_powers::<E::G1>(tau, n);
    let elapsed_time = now.elapsed();
    println!("Running compute_g_powers G1() took {} seconds.", elapsed_time.as_secs());

    let now = Instant::now();
    let _ = compute_g_powers::<E::G2>(tau, n + 1);
    let elapsed_time = now.elapsed();
    println!("Running compute_g_powers G2() took {} seconds.", elapsed_time.as_secs());
    ...
    let elapsed_time = now.elapsed();
    println!("Running compute_qs took {} seconds.", elapsed_time.as_secs());
}
