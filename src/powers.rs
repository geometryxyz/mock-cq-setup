...
pub fn compute_g_powers<G: CurveGroup>(tau: G::ScalarField, n: usize, path: Option<&str>) -> Option<Vec<G::Affine>> {
...
mod powers_test { 
    ...
    fn sanity_srs<E: Pairing>(
        n: usize,
        tau: E::ScalarField,
    ) -> Vec<E::G1Affine> {
        let powers_of_tau: Vec<E::ScalarField> = std::iter::successors(Some(E::ScalarField::one()), |p| Some(*p * tau))
            .take(n + 1)
            .collect();
    ...
    #[test]
    fn test_srs() {
        use ark_bn254::{Bn254, G1Projective, Fr};
        let k = 1; 
        let n = 1 << k; 
